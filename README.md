...
| **Tools** | Its own section with its own permissions, not a corner of the parts store — a part is used up, a tool is borrowed, and somebody can hold one without the other. Issue a tool to somebody at a room or area, take it back in, and read every journey it has made. A tool cannot be issued while it is already out (the database refuses it) and one still out cannot be retired. Anything not returned within 24 hours is chased once, by an hourly sweep, to whoever keeps the register. |
