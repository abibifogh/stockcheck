-- Maintenance stores: parts and consumables issued to rooms and areas.
--
-- Deliberately separate tables from the breakfast side rather than a shared
-- "items" table with a type column. The two stores answer different questions —
-- breakfast asks "what did each guest cost", maintenance asks "what did each
-- room cost" — and sharing a table would mean every query in both systems
-- carrying a filter it must never forget.
--
...
CREATE TABLE IF NOT EXISTS mx_items (
  ...
  -- The amount you want on the shelf. Falling below it raises a restock alert.
  par_level         REAL NOT NULL DEFAULT 0,
  opening_stock     REAL NOT NULL DEFAULT 0,
  default_unit_cost REAL NOT NULL DEFAULT 0,
  -- Shown on the issue screen without searching. The handful of things that go
  -- out every week; everything else hides behind "All items".
...
-- Where work happens: guest rooms and everything that is not a guest room.
CREATE TABLE IF NOT EXISTS mx_areas (
  id         INTEGER PRIMARY KEY AUTOINCREMENT,
  name       TEXT NOT NULL UNIQUE,
  kind       TEXT NOT NULL DEFAULT 'room',   -- 'room' or 'area'
...
-- One row per release of stock. `day` is carried alongside `at` because every
-- report groups by day and deriving it from a timestamp in SQLite on every
-- query is both slower and easier to get wrong across timezones.
...
-- Physical counts, for comparing what is on the shelf against what the book
...
INSERT OR IGNORE INTO settings (key, value) VALUES
  ('mx_enabled', '1'),
  -- Warn when the shelf holds less than this many days of typical use.
  ('mx_low_cover_days', '7');

-- ---------------------------------------------------------------------------
-- A starting point. All of it is editable, and none of it is load-bearing.
-- ---------------------------------------------------------------------------
...
-- The places that are not guest rooms. Rooms themselves are added in Setup,
-- where a whole floor can be created in one go.
